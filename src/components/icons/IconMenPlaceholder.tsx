import React, { FC } from 'react'

interface IconMenPlaceholderProps {
  size?: number
}

export const IconMenPlaceholder: FC<IconMenPlaceholderProps> = ({ size = 28 }) => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width={size}
      height={size}
      fill='none'
      viewBox='0 0 28 28'
      id='UserMenPlaceholder'
    >
      <g clipPath='url(#clip0)'>
        <path
          d='M23.0335 21.0525L18.564 19.9349L18.1836 18.4135C19.3795 17.2294 20.1985 15.603 20.4411 13.9346C21.1038 13.748 21.6171 13.1798 21.7058 12.467L21.9974 10.1336C22.0593 9.6413 21.9076 9.14315 21.5809 8.7698C21.3896 8.55045 21.1481 8.3848 20.8809 8.2833L20.9883 6.08645L21.4246 5.64895C22.0815 4.95009 22.6263 3.75195 21.4876 2.01359C20.6126 0.677852 19.1264 0 17.0684 0C16.2564 0 14.3874 0 12.6467 1.169C7.67665 1.27285 7 3.58165 7 5.83335C7 6.3572 7.12715 7.5367 7.21115 8.24835C6.9113 8.34285 6.6395 8.51785 6.42715 8.75935C6.09465 9.1362 5.94065 9.6367 6.0025 10.1349L6.29415 12.4682C6.3898 13.2289 6.9673 13.8274 7.69765 13.9697C7.93915 15.5739 8.71615 17.1477 9.84315 18.3121L9.43715 19.9372L4.96765 21.0549C2.04165 21.784 0 24.3996 0 27.4166C0 27.7386 0.261352 28 0.583352 28H27.4167C27.7387 28 28.0001 27.7364 28.0001 27.4144C28 24.3996 25.9584 21.784 23.0335 21.0525Z'
          fill='white'
        />
      </g>
      <defs>
        <clipPath id='clip0'>
          <rect width='28' height='28' fill='white' />
        </clipPath>
      </defs>
    </svg>
  )
}
